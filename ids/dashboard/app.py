...
def safe_json_response(data, status_code=200):
    ...
    try:
        ...
        error_response = json.dumps({"error": "Failed to serialize response", "details": str(e)})
...
class DashboardApp:
    ...
    def __init__(self, detector_engine=None):
        ...
        self.app = Flask(
            __name__, static_folder=str(dashboard_dir / "static"), template_folder=str(dashboard_dir / "templates")
        )
    ...
    def _register_routes(self):
        ...
        def get_stats():
            try:
                ...
                response = {"database": stats, "engine": engine_stats, "timestamp": datetime.now().isoformat()}
        ...
        def get_interfaces():
            try:
                ...
                try:
                    configured = (
                        self.detector_engine.packet_capture.interface
                ...
                return jsonify(
                    {"available_interfaces": interfaces, "configured_interface": configured, "os": platform.system()}
                )
            except Exception as e:
        ...
        def capture_status():
            try:
                ...
                return jsonify(
                    {"running": running, "interface": iface, "bpf_filter": bpf, "promiscuous": promisc, "stats": stats}
                )
            except Exception as e:
        ...
        def capture_test():
            ...
            try:
                ...
                # If we have an engine, put it through normal pipeline or store synchronously if engine is not running
                if self.detector_engine:
                    try:
                        # If the engine is running, enqueue for normal processing
                        if getattr(self.detector_engine, "_is_running", False):
                        ...
                        # If engine not running (e.g., during tests), store directly into packet DB synchronously
                        else:
                            try:
                                packet_id = self.detector_engine.packet_db_manager.store_packet(test_packet)
                                return jsonify({"ok": True, "packet_id": packet_id})
                            except Exception as e:
                                logger.error(f"Failed to store test packet via engine DB: {e}", exc_info=True)
                                # Fallback to storing via dashboard DB manager
                    except Exception as e:
        ...
        def handle_request_alerts(data):
            try:
                ...
                emit("alerts_update", {"alerts": safe_alerts, "timestamp": datetime.now().isoformat()})
            except Exception as e:
        ...
        try:
            with self._clients_lock:
                if self._connected_clients > 0:
                    self.socketio.emit(
                        "new_alert",
                        {"alert": self._fix_alert_dict(alert), "timestamp": datetime.now().isoformat()},
                        broadcast=True,
                    )
