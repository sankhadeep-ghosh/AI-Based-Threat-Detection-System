...
class DetectorEngine:
    ...
    def __init__(
        ...
        # Control frequency of packet logging (every Nth packet)
        try:
            self.packet_log_every = int(config.get("logging", {}).get("packet_log_every", 1) or 1)
        except Exception:
            self.packet_log_every = 1

        self.packet_capture = PacketCapture(interface=interface, promiscuous=promiscuous, filter_exp=filter_exp)
        ...
        logger.info(f"DetectorEngine initialized | Signature={use_signature} | Anomaly={use_anomaly}")
    ...
    def start(self) -> None:
        ...
        capture_thread = Thread(target=self.packet_capture.start, name="PacketCaptureInit", daemon=True)
    ...
    def _packet_processor(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if self.log_packets:
                    try:
                        if self.packet_log_every <= 1 or (packet_count % self.packet_log_every == 0):
                            ...
                            try:
                                ...
                                direction = "unknown"
                            logger.info(
                                f"[Packet] {direction.upper():8} {src}{(':'+str(src_port)) if src_port else ''} -> {dst}{(':'+str(dst_port)) if dst_port else ''} | {proto.upper()} | len={length}"
                            )
                    except Exception:
                        # Misconfigured packet_log_every; fall back to logging every packet
                        src = features.get("source_ip", "unknown")
                        dst = features.get("dest_ip", "unknown")
                        proto = features.get("protocol", "unknown")
                        src_port = features.get("source_port")
