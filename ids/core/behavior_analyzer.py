...
class PortScanTracker:
    ...
    def _cleanup_old_entries(self, now: datetime) -> None:
        ...
        cutoff = now - timedelta(seconds=self.window)
        expired_ips = [
            ip for ip, timestamp in self.timestamps.items()
            if timestamp < cutoff
        ]
...
class BruteForceTracker:
    ...
    def _cleanup_old_entries(self, now: datetime) -> None:
        ...
        cutoff = now - timedelta(seconds=self.window)
        expired_targets = [
            target for target, timestamp in self.timestamps.items()
            if timestamp < cutoff
        ]
...
class BehaviorAnalyzer:
    ...
    def __init__(self, thresholds_file: str = "thresholds.yaml"):
        ...
        self.dns_tracker = SlidingWindowCounter(
            window_seconds=self.thresholds["dns_amplification"]["time_window"]
        )
    ...
    def _load_thresholds(self, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "brute_force": {"max_failed_attempts": 5, "time_window": 60, "target_ports": [22, 23, 3389, 21], "severity": "high"},
